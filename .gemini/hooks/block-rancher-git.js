...
function main() {
  ...
  const isExecutingHooksManually = command.includes('.gemini/hooks/') || command.includes('.gemini/hooks');
  if (isExecutingHooksManually) {
    console.log(
      JSON.stringify({
        decision: 'deny',
        reason:
          '🔒 Security Policy Violation: Manual execution of enforcer hook scripts inside .gemini/hooks/ is strictly prohibited.\n\n' +
          'Enforcer hooks are part of the secure system pipeline and must only be executed automatically by the Gemini CLI lifecycle.',
        systemMessage: '🔒 Security Block: Manual execution of enforcer hooks is prohibited.',
      }),
    );
    process.exit(0);
  }
  ...
  const isManipulatingApproval =
    /\b(echo|cat|touch|rm|mv|cp|write|tee|vim|vi|nano|printf|sed|awk)\b.*\breview-approval\.json\b|>>?[^>]*\breview-approval\.json\b/.test(
      commandClean,
    );
