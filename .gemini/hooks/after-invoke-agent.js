  ...
  if (agentName === 'testing_agent') {
    ...
    if (isSuccess) {
      ...
      console.log(
        JSON.stringify({
          decision: 'allow',
          systemMessage: `❌ Gate 2 Rejected: Testing sub-agent reported failures. Gate 2 signature revoked/missing. Report saved to ~/.gemini/tmp/${repoName}/logs/testing_agent_report.md`,
        }),
      );
      process.exit(0);
    }
    ...
    if (isSuccess) {
      ...
      console.log(
        JSON.stringify({
          decision: 'allow',
          systemMessage: `❌ Gate 3 Rejected: Review sub-agent reported violations. Gate 3 signature revoked/missing. Report saved to ~/.gemini/tmp/${repoName}/logs/review_agent_report.md`,
        }),
      );
      process.exit(0);
    }
