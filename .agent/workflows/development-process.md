...
4. **Acquire, Edit, or Create Architectural Plan:** Following `.agent/rules/plans.instructions.md`, verify if an existing plan covers the target domain.
   - **If an existing plan covers the domain:** You MUST NOT create a new plan file. Instead, _edit_ and _adapt_ the existing plan, modifying its top-half blueprint/specification and expanding/re-opening the bottom-half implementation checklist.
