...
export default async ({ github, context, core, process }) => {
  ...
  if (pr.draft) {
    core.setFailed(
      `PR #${prNumber} is currently in draft mode. Verification cannot proceed until the PR is marked as ready for review.`,
    );
    return;
  }
...
async function verifyPullRequest({ github, context, core, pr, owner, repo, checkCI }) {
  ...
  if (checkCI) {
    ...
    const { data: checks } = await withRetry(core, () =>
    ...
    const failedRuns = relevantCheckRuns.filter(
    ...
    if (totalRuns === 0) {
      ...
      reasons.push(
        `- **CI Checks**: Some CI check runs failed:\n` +
  ...
  if (unverifiedCommits.length > 0) {
    reasons.push(
      `- **Verified Commits**: The following commit(s) are not signed or verified:\n` +
        unverifiedCommits
  ...
  const reviews = await withRetry(core, () =>
  ...
  for (const review of Object.values(latestReviews)) {
    ...
    try {
      const { data: permData } = await withRetry(core, () =>
  ...
  const gqlResult = await withRetry(core, () =>
    github.graphql(gqlQuery, {
      owner,
      repo,
  ...
  if (unresolvedThreads.length > 0) {
    reasons.push(
      `- **Review Comments**: All review comments must be resolved. There are currently ${unresolvedThreads.length} unresolved thread(s):\n` +
        unresolvedThreads
          .map((t) => {
