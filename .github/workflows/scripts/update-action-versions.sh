...
Options:
  ...
  --validate           Check if all actions are up to date. Exits 1 if updates are needed.
...
update_workflow_releases() {
    ...
    for repo in ${repos}; do
      ...
      if [[ "${mode}" == "validate" ]]; then
        if ! cmp -s "${workflow}" "${tmp_file}"; then
          echo "Error: Outdated action version found for ${repo} in ${workflow}. Latest is ${tag} (${sha})." >&2
          outdated=true
        fi
        rm -f "${tmp_file}"
      else
  ...
  if [[ "${mode}" == "validate" ]]; then
    if [[ "${outdated}" == "true" ]]; then
      echo "Validation failed: One or more actions are out of date." >&2
      return 1
    else
      echo "Validation passed: All actions are up to date." >&2
      return 0
    fi
  fi
}
...
main() {
  ...
  update_workflow_releases "${action}"
