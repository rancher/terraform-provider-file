...
test('verify-pr-requirements.mjs tests', async (t) => {
  ...
  await t.test('Standard PR with human-only approval succeeds (no AI review required)', async () => {
    const coreLogs = [];
    let failedReason = '';
    const core = {
  ...
  await t.test('Dependabot PR succeeds with at least 1 AI review', async () => {
    const coreLogs = [];
    let failedReason = '';
    const core = {
      info: (msg) => coreLogs.push(msg),
