...
test("verify-pr-requirements.mjs tests", async (t) => {
  ...
  await t.test(
    "Standard PR with human-only approval fails (missing AI review)",
    async () => {
      const coreLogs = [];
      let failedReason = "";
      const core = {
                ...
                nodes: [
                  {
                    ...
                    comments: {
                      nodes: [
                        { body: "looks good", author: { login: "reviewer" } },
                      ],
                    },
                  },
                ],
  ...
  await t.test("Standard PR with human and AI approvals succeeds", async () => {
    ...
    assert.ok(
      coreLogs.includes(
        "PR #400 meets all verification requirements and is ready to merge!",
      ),
    );
  });

  await t.test(
    "Standard PR with human approval and AI review via conversation comment succeeds",
    async () => {
      const coreLogs = [];
      let failedReason = "";
      const core = {
  ...
  await t.test(
    "Dependabot PR succeeds, bypassing both human and AI review requirements",
    async () => {
      const coreLogs = [];
      let failedReason = "";
      const core = {
      ...
      assert.ok(
        coreLogs.includes(
          "PR #401 meets all verification requirements and is ready to merge!",
        ),
      );
    },
  );
