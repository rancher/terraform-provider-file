...
test('git-helpers.js: master consolidated helper unit tests', async (t) => {
  ...
  const tempHome = path.resolve(`/tmp/gemini-helpers-test-${uniqueId}`);
  ...
  await t.test('getForkOwner parses origin remote url correctly', () => {
    execSync('git remote add origin https://github.com/my-fork/terraform-provider-file.git', { cwd: tempHome });
  ...
  await t.test('verifyPushSafety allows safe remotes and blocks unsafe remotes', () => {
    ...
    execSync('git remote add rancherremote https://github.com/rancher/terraform-provider-file', { cwd: tempHome });
    assert.throws(() => {
